...
void Interpretation(VEZ *vez, float RMS, int max_iteration) {
    ...
    vez->q16 = (int) ceil((6 * log(vez->curve.AB2[vez->curve.n - 1] / vez->curve.AB2[0])) / log(10)) + 1;
...
*/
    ...
    Spline(&(vez->curve.AB2[0]), &(vez->curve.fRok[0]), vez->curve.n, absc6, &(RF[1]), vez->q16);
...
-----------------------------------------------------------------------*/
void Spline(float *x, float *y, int n, float *x_, float *y_, int n_) {
    float l[MAX_SAMPLES], m[MAX_SAMPLES], r[MAX_SAMPLES], s[MAX_SAMPLES];
    int i, j, k, cnt;
