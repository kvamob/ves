...
namespace VesExplore
{
	...
	public class VesXml
	{
		...
		/// Конструктор
		/// </summary>
		public VesXml(NewFileSettings settings)
		{
		...
		private void initNewDocument(NewFileSettings settings)
		{
...
//			float dx		= Convert.ToSingle(settings.DX,en);
//			float dy		= Convert.ToSingle(settings.DY,en);
			...
			{
				string name;
				if (settings.TypeProf)
					name = String.Format("{0} {1}", settings.ProfName, i);
				else
				...
				string type = (settings.TypeProf) ? "profil" : "curve";
				...
				for (int j = 0; j < nCurves; j ++)
				{
					...
					elem.SetAttribute("name", name );
					elem.SetAttribute("date", settings.Date.ToShortDateString());
...
//			xmlDoc.Save("new.xml");
		...
		/// <returns>Добавленный узел  Xml</returns>
		public XmlNode AddCurve(AddCurveSettings settings, XmlNode selectedXmlNode)
		{
			CultureInfo	en = new CultureInfo("en-US");	// Для правильного преобразования string в float
			...
			{
				case "profil"	: parentXmlNode = selectedXmlNode.SelectSingleNode("curves"); break;
				case "curve"	: parentXmlNode = selectedXmlNode.ParentNode; break;
			}
			...
			curve.AppendChild(elem);
			elem = m_xmlDoc.CreateElement("device");
			elem.InnerText = settings.Device;
			curve.AppendChild(elem);
			elem = m_xmlDoc.CreateElement("image");
			curve.AppendChild(elem);
