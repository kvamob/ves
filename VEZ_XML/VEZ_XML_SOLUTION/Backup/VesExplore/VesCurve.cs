...
namespace VesExplore
{
	...
	public class VesCurve
	{
		...
		private float	_marginHoriz = 40;	// Отступы рамки от края рисунка по горизонтали, пикс
		private float	_marginVert  = 30;	// Отступы рамки от края рисунка по вертикали, пикс
		private float	_module      = 50;	// Модуль логарифмического масштаба, пикс
		...
		/// Имя кривой
		/// </summary>
		[Category("Common"),Description("Имя кривой"),Browsable(true),ReadOnly(false)]
		...
		/// Описание кривой
		/// </summary>
		[Category("Common"),Description("Описание кривой"),Browsable(true),ReadOnly(false)]
		...
		/// Имя оператора
		/// </summary>
		[Category("Persons"),Description("Оператор"),Browsable(true),ReadOnly(false)]
		...
		/// Имя вычислителя
		/// </summary>
		[Category("Persons"),Description("Вычислитель"),Browsable(true),ReadOnly(false)]
		...
		/// Погода
		/// </summary>
		[Category("Misc"),Description("Погода"),Browsable(true),ReadOnly(false)]
		...
		/// Дата измерений
		/// </summary>
		[Category("Misc"),Description("Дата"),Browsable(true),ReadOnly(false)]
		public string Date
		{
			get
			{
		...
		/// Аппаратура
		/// </summary>
		[Category("Misc"),Description("Аппаратура"),Browsable(true),ReadOnly(true)]
		...
		/// </summary>
		[Category("Misc"),Description("Кол-во слоев в модели"),Browsable(true),ReadOnly(true)]
		public int NLayers
		{
			get
			{
		...
		/// Координата X кривой
		/// </summary>
		[Category("Location"),Description("Координата X кривой"),Browsable(true),ReadOnly(false)]
		public  float X
		{
			get
			{
		...
		/// Координата Y кривой
		/// </summary>
		[Category("Location"),Description("Координата Y кривой"),Browsable(true),ReadOnly(false)]
		public  float Y
		{
			get
			{
		...
		/// Координата Z кривой
		/// </summary>
		[Category("Location"),Description("Координата Z кривой"),Browsable(true),ReadOnly(false)]
		public  float Z
		{
			get
			{
		...
		public void AutoInterp(float desiredRMS, int maxIteration )
		{
			...
			Trace.WriteLine( nsamples, "nsamples" );
			Trace.WriteLine( this._nLayers, "nlayers" );
		...
		public void Draw(Graphics clientDC, int width, int height)
		{
			...
			_module = (float) Math.Min((width - 2*_marginHoriz)/_AB2Decads, (height - 2*_marginVert)/_RokDecads);
		...
		private void drawLogBlank(Graphics g)
		{
			...
			while ( ptBott.X < (sizeBord.Width + ptOrig.X) )
			{
				++ j;
				x = x0*j;
				ptBott.X = ptOrig.X + Convert.ToSingle(Math.Log10(x/x0)*_module) + _module*k;
				ptTop.X  = ptBott.X;
				if ( j == 10 )
				{
					j = 1; 
			...
			while ( ptLeft.Y > ptOrig.Y )
			{
				++ j;
				y = y0*j;
				ptLeft.Y = ptOrig.Y + sizeBord.Height - Convert.ToSingle(Math.Log10(y/y0)*_module) - _module*k;
				ptRight.Y  = ptLeft.Y;
				if ( j == 10 )
				{
					j = 1; 
			...
			ptLeft.X -= stringSize.Width;							// Подправим положение
			...
			ptTop.X = _marginHoriz + sizeBord.Width/2;
			ptTop.Y = _marginVert + sizeBord.Height + fontAxis.GetHeight();
			fmt.Dispose();
		...
		private void drawVesCurve(Graphics g, float [] AB2, float [] fRok, float [] tRok, int nSamples)
		{
			...
			PointF ptOrigLB = new PointF(_marginHoriz, _marginVert + sizeBord.Height );
			...
			float _ab2 = Convert.ToSingle( Math.Log10((AB2[0]/ab2_0))*_module );
			float _rok = Convert.ToSingle( Math.Log10((fRok[0]/rok_0))*_module );
			PointF pt1 = new PointF(ptOrigLB.X + _ab2, ptOrigLB.Y - _rok);
			PointF pt2 = new PointF();
			g.DrawEllipse(pen,pt1.X-2,pt1.Y-2,4,4);
			for ( int i = 1; i < nSamples; i ++ )
			{
				_ab2 = Convert.ToSingle( Math.Log10((AB2[i]/ab2_0))*_module );
				_rok = Convert.ToSingle( Math.Log10((fRok[i]/rok_0))*_module );
			...
			Pen   pen2 = new Pen(Color.Red, 3F); // перо
			pen1.DashStyle = DashStyle.Dot;
			_ab2 = Convert.ToSingle( Math.Log10((AB2[0]/ab2_0))*_module );
			_rok = Convert.ToSingle( Math.Log10((tRok[0]/rok_0))*_module );
			pt1.X = ptOrigLB.X + _ab2;
			pt1.Y = ptOrigLB.Y - _rok;
			g.DrawEllipse(pen2,pt1.X-2,pt1.Y-2,4,4);
			for ( int i = 1; i < nSamples; i ++ )
			{
				_ab2 = Convert.ToSingle( Math.Log10((AB2[i]/ab2_0))*_module );
				_rok = Convert.ToSingle( Math.Log10((tRok[i]/rok_0))*_module );
				pt2.X = ptOrigLB.X + _ab2;
				pt2.Y = ptOrigLB.Y - _rok;
		...
		private void drawLayers(Graphics g, float [] h, float [] r, int nLayers)
		{
			//Размеры рамки 
			SizeF  sizeBord = new SizeF(_module*_AB2Decads,_module*_RokDecads);
			// Левый нижний угол рамки
			...
			// Значения Rok и AB/2 в левом нижнем углу рамки
			float h_0 = Convert.ToSingle( Math.Pow(10,Math.Floor( Math.Log10(Convert.ToDouble(_AB2Min)))) );
			float r_0 = Convert.ToSingle( Math.Pow(10,Math.Floor( Math.Log10(Convert.ToDouble(_RokMin)))) );
			
			// Ломаная
			Pen   pen = new Pen(Color.Blue, 3F); //  перо

			float _h = Convert.ToSingle( Math.Log10(( (h[0]*10)/h_0))*_module );
			float _r = Convert.ToSingle( Math.Log10((   r[0]  /r_0))*_module );
			
			PointF pt1 = new PointF(ptOrigLB.X, ptOrigLB.Y - _r);
			PointF pt2 = new PointF(ptOrigLB.X + _h, ptOrigLB.Y - _r);
//			g.DrawEllipse(pen,pt1.X-2,pt1.Y-2,4,4);
			
			for ( int i = 1; i < nLayers; i ++ )
			{
				_h = Convert.ToSingle( Math.Log10((h[i]/h_0))*_module );
				_r = Convert.ToSingle( Math.Log10((r[i]/r_0))*_module );
				pt2.X = ptOrigLB.X + _h;
				pt2.Y = ptOrigLB.Y - _r;
				g.DrawLine(pen,pt1,pt2);
				pt1.X = pt2.X;
...
//				g.DrawEllipse(pen,pt1.X-2,pt1.Y-2,4,4);
		...
		/// <summary>
		/// Напечатать кривую ВЭЗ
		/// </summary>
		/// <param name="clientDC">Контекст устройства, на котором будем печатать</param>
		/// <param name="clientDC">Настройки печати</param>
		public void Print(Graphics clientDC, PrintingSettings ps)
		{
			int nsamples = this._samplesTable.Rows.Count;
			float [] AB2  = new float [nsamples];	// Массив значений AB/2
			float [] fRok = new float [nsamples];	// Массив измер. значений каж. сопр.
			...
			_module = ps.Module;
			_marginHoriz = ps.LeftMargin;
			_marginVert  = ps.TopMargin;

			clientDC.PageUnit = GraphicsUnit.Millimeter;
			
//			// Чтобы не "мерцало" изображение, рисовать будем в битмапе
//			Bitmap		bmp = new Bitmap(width, height);
//			Graphics	g =  Graphics.FromImage(bmp);

			printLogBlank(clientDC);
			printVesCurve(clientDC, AB2, fRok, tRok, nsamples);

//			printLogBlank(g);
//			printVesCurve(g, AB2, fRok, tRok, nsamples);
//			// А теперь битмап перепишем в клиентскую область рисования
//			clientDC.DrawImage(bmp,0,0);
		...
		/// <summary>
		/// Печатать логарифмический бланк
		/// </summary>
		/// <param name="g">Контекст устройства</param>
		private void printLogBlank(Graphics g)
		{
//			g.Clear(Color.White);
			Pen   penDot   = new Pen(Color.Black, 0.2F); 
			penDot.DashStyle = DashStyle.Dot;

			Pen   penSolid = new Pen(Color.Black, 0.2F); 
			...
			fmt.Alignment = StringAlignment.Center;
			string hdrStr = string.Format("Участок: {0}\nПрофиль: {1}\nКривая: {2}",
				_areaName, _profilName, _curveName);
//			string hdrStr = string.Format("Кривая: {0}", _curveName);
			g.DrawString(hdrStr, fontHdr, brush, ptTop, fmt);
			fontHdr.Dispose();
		...
		/// <param name="tRok">Массив вычисленных значений сопротивления</param>
		/// <param name="nSamples">Кол-во отсчетов на кривой</param>
		private void printVesCurve(Graphics g, float [] AB2, float [] fRok, float [] tRok, int nSamples)
		{
			//Размеры рамки 
			SizeF  sizeBord = new SizeF(_module*_AB2Decads,_module*_RokDecads);
			// Левый нижний угол рамки
			...
			// Полевая кривая
			Pen   pen = new Pen(Color.Blue, 0.5F); //  перо
			float _ab2 = Convert.ToSingle( Math.Log10((AB2[0]/ab2_0))*_module );
			float _rok = Convert.ToSingle( Math.Log10((fRok[0]/rok_0))*_module );
			PointF pt1 = new PointF(ptOrigLB.X + _ab2, ptOrigLB.Y - _rok);
			PointF pt2 = new PointF();
			g.DrawEllipse(pen, pt1.X-0.5F, pt1.Y-0.5F, 1, 1);
			for ( int i = 1; i < nSamples; i ++ )
			{
				_ab2 = Convert.ToSingle( Math.Log10((AB2[i]/ab2_0))*_module );
				_rok = Convert.ToSingle( Math.Log10((fRok[i]/rok_0))*_module );
			...
			// Вычисленная кривая
			Pen   pen1 = new Pen(Color.Red, 0.5F); // перо
			Pen   pen2 = new Pen(Color.Red, 0.5F); // перо
			pen1.DashStyle = DashStyle.Dot;
			_ab2 = Convert.ToSingle( Math.Log10((AB2[0]/ab2_0))*_module );
			_rok = Convert.ToSingle( Math.Log10((tRok[0]/rok_0))*_module );
			pt1.X = ptOrigLB.X + _ab2;
			pt1.Y = ptOrigLB.Y - _rok;
			g.DrawEllipse(pen2, pt1.X-0.5F, pt1.Y-0.5F, 1, 1);
			for ( int i = 1; i < nSamples; i ++ )
			{
				_ab2 = Convert.ToSingle( Math.Log10((AB2[i]/ab2_0))*_module );
				_rok = Convert.ToSingle( Math.Log10((tRok[i]/rok_0))*_module );
