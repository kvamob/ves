...
#define FILENAMESIZE 256       // Макс. длина имени файла
...
WNDCLASS wc;
 ...
 wc.lpfnWndProc   = FrameWndProc;    // Пpисоединенная функция класса
...
// wc.hbrBackground = COLOR_APPWORKSPACE+1;          // Цвет фона по умолч.
// wc.hbrBackground = COLOR_WINDOW;          // Цвет фона по умолч.
 ...
 if ( !RegisterClass(&wc) ) return FALSE; // Заpегистpиpовать класс

  /* Регистрация класса вспомогательного окна. */
 wc.style         = CS_OWNDC;        // Cтиль класса
 wc.lpfnWndProc   = ProgressWndProc; // Пpисоединенная функция класса
 wc.cbClsExtra    = 0;               // Дополнительные данные для класса
 wc.cbWndExtra    = 0;               // Дополнительные данные для окна
 wc.hInstance     = hInstance;       // Идентификатоp владельца класса
// wc.hIcon         = LoadIcon(hInstance,(LPSTR)"ICON_VES"); // Иконка по умолч.
...
/* ---------------------------------------------------------------
 ...
 --------------------------------------------------------------- */
#pragma argsused   // Отключим предупреждения о неиспольз. арг.
LONG DoCreate(HWND hWnd, WORD wMsg, WORD wParam, LONG lParam)
{
...
WORD status, status1, status2;
 ...
 if ( nProfilOpen )  // Профиль открыт
  { status = MF_ENABLED;
   ...
   EnableMenuItem( wParam, IDM_SAVE, status );     // Установим статус
   EnableMenuItem( wParam, IDM_SAVE_AS, status );
   EnableMenuItem( wParam, IDM_AUTO, status );
   EnableMenuItem( wParam, IDM_MANUAL, status );

   EnableMenuItem( wParam, IDM_NEXT, status1 ); // Установим статус
   EnableMenuItem( wParam, IDM_LAST, status1 ); // Установим статус
   EnableMenuItem( wParam, IDM_PREV, status2 ); // Установим статус
   EnableMenuItem( wParam, IDM_FIRST,status2 ); // Установим статус
...
//nProfilOpen = 0;
if (GetOpenFileName( &ofn ))  // Открыть диалог Open
 {
...
LONG DoMenuInfo(HWND hWnd, WORD wMsg, WORD wParam, LONG lParam)
{
FARPROC lpProcInfo; // Адрес процедуры
...
LONG DoMenuAbout(HWND hWnd, WORD wMsg, WORD wParam, LONG lParam)
{
FARPROC lpProcAbout; // Адрес процедуры
...
LONG DoMenuInterpAuto(HWND hWnd, WORD wMsg, WORD wParam, LONG lParam)
{
FARPROC lpProcAuto; // Адрес процедуры
...
LONG DoMenuInterpMan(HWND hWnd, WORD wMsg, WORD wParam, LONG lParam)
{
FARPROC lpProcMan; // Адрес процедуры
...
LONG DoMenuComputeCurve(HWND hWnd, WORD wMsg, WORD wParam, LONG lParam)
{
FARPROC lpProcCTC; // Адрес процедуры
